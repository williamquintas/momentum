...
- Example:
  ...
  export const useInfiniteGoals = (filters?: GoalFilters) => {
    return useInfiniteQuery({
      ...
      queryFn: ({ pageParam = 1 }) => goalService.getPaginated({ page: pageParam, pageSize: 20 }),
...
- Example:
  ...
  export const selectFilteredGoals = createSelector([selectGoals, selectGoalFilters], (goals, filters) => {
    return goals.filter((goal) => goal.title.includes(filters.search || ''));
  });
