...
export function validateCompletionEligibility(goal: Goal): ValidationResult {
  ...
  switch (goal.type) {
    case 'quantitative':
      return goal.currentValue >= goal.targetValue ? { valid: true } : { valid: false, reason: 'Target not reached' };

    case 'binary':
      return goal.achieved ? { valid: true } : { valid: false, reason: 'Goal not achieved' };
...
export function calculateCompletionMetrics(goal: Goal, history: ProgressUpdate[]): CompletionMetrics {
...
## Accessibility & Performance Testing (from 017-add-a11y-perf-nfrs)

### Running Accessibility Tests

```bash
# Run axe-core on completion components
npm run test:a11y

# Run a specific component test
npx vitest run src/features/goals/components/__tests__/CompleteGoalDialog.test.tsx
```

### Manual Accessibility Checklist for Completion

**Keyboard Navigation**:

1. Open completion dialog using Tab/Enter
2. Verify Tab cycles through all elements in logical order
3. Verify Escape closes the dialog
4. Verify Enter on "Complete Goal" button triggers completion

**Screen Reader**:

1. Enable NVDA/VoiceOver
2. Open completion dialog
3. Verify completion criteria are announced
4. Verify success message announced after completion
5. Verify error messages associated with form fields

**Color Contrast**:

1. Use browser dev tools: Elements panel → Accessibility
2. Verify completion dialog text ≥4.5:1 contrast
3. Verify buttons/links ≥3:1 contrast
...
# Manual test: Complete goal flow should take <2s
```

### Completion-Specific Performance Checks

- Completion eligibility check: <50ms
- Metrics calculation: <200ms
- Dialog open time: <200ms
- Goal status update: <100ms
- Total completion flow: <2s
