...
- Example:
  ...
  export const useQueryWithLogging = <TData, TError>(options: UseQueryOptions<TData, TError>) => {
    return useQuery({
      ...options,
      onError: (error) => {
...
- Example:
  ...
  export const loggerMiddleware = <T>(config: StateCreator<T>): StateCreator<T> => {
    return (set, get, api) => {
      return config(
        (...args) => {
